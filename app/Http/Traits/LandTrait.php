...
trait LandTrait
{
    ...
    public function Landindex( Land_US $land)
    {
        $this->land = $land;
        $address = $this->addressLandSearch();
        $building = $this->buildingLandSearch($this->land->building);
        $land = $this->landLandSearch($this->land);
        $price = '';
        if(!is_null($this->land->price)){
            $price = $this->priceLandSearch($this->land->price);
        }

        $terms = '';
        if(!is_null($this->land->price)){
            $terms = $this->termsLandSearch($this->land->terms);
        }

        $landplot = '';
        if(!is_null($this->land->price)){
            $landplot = $this->landplotLandSearch($this->land->land_plot);
        }
        $quick_search = $address.$this->separator.$building.$this->separator.$land.$this->separator.$price.$this->separator.$terms.$this->separator.$landplot;
        $this->land->quick_search = $quick_search;
        $this->land->save();
    }

    public function addressLandSearch() : string
    {
        $house_name = '№'.$this->land->CommerceAddress()->house_id;
        $cityName = 'г. ';
        if (!is_null($this->land->CommerceAddress()->city->type)){
            $cityName = $this->land->CommerceAddress()->city->type->name.' ';
        }
        $street_name = '';
        if(!is_null($this->land->CommerceAddress()->street) && !is_null($this->land->CommerceAddress()->street->street_type)){
            $street_name = $this->land->CommerceAddress()->street->full_name();
        }
        $district_name = '';
        if(!is_null($this->land->CommerceAddress()->district)){
            $district_name = $this->land->CommerceAddress()->district->name;
        }
        $microarea_name = '';
        if(!is_null($this->land->CommerceAddress()->microarea)){
            $microarea_name = $this->land->CommerceAddress()->microarea->name;
        }
        $landmark_name = '';
        if(!is_null($this->land->building->landmark)){
            $landmark_name = $this->land->building->landmark->name;
        }

        $section = '';
        if (!is_null($this->land->building->section_number)){
            $section = 'корпус '.$this->land->building->section_number;
        }
        $land_number = '';
        if (!is_null($this->land->flat_number)){
            $land_number = 'кв.'.$this->land->flat_number;
        }

        return $house_name.$this->separator
            .$cityName.$this->separator
    ...
    public function buildingLandSearch(Building $building) : string
    {
        $typeOfBuilding = '';
    ...
    public function landLandSearch(Land_US $land) : string
    {
    ...
    public function priceLandSearch(ObjectPrice $price) : string
    {
        $object_doc = '';
    ...
    public function termsLandSearch(ObjectTerms $termsSale) : string
    {
        $exclusive = '';
    ...
    public function landplotLandSearch(LandPlot $landPlot) : string
    {
